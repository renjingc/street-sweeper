...
class Map{
	public:
		...
		inline int getMapSizeY() const {return m_mapSizeY;}
                inline int getSizeX2() const {return m_sizeX2;}
                inline int getSizeY2() const {return m_sizeY2;}
...
Map<Cell,Storage,isClass>::Map(const Point& center, double xmin, double ymin, double xmax, double ymax, double delta):
	...
	m_mapSizeY=m_storage.getYSize()<<m_storage.getPatchSize();
        ...
        //m_sizeX2=(int)floor((m_center.x-xmin)/m_delta);
        //m_sizeY2=(int)floor((m_center.y-ymin)/m_delta);
}
...
IntPoint Map<Cell,Storage,isClass>::world2map(const Point& p) const{
    ...
    //return IntPoint( (int)floor((p.x-m_center.x)/m_delta)+m_sizeX2, (int)floor((p.y-m_center.y)/m_delta)+m_sizeY2);
}
