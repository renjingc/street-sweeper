...
//        string imuPString=strtool::numToString<int>(hl->frameNum)+" updateForth: "+strtool::numToString<int>(p.fourDirection)+" "+strtool::numToString<double>(errCurAndLastImuTh);
//        logUpdateDataFile<<imuPString<<endl;
    }
    if(errCurAndLastImuX>thresX||errCurAndLastImuY>thresY||(errCurAndLastImuTh>(thresTh-PI/32)&&errCurAndLastImuTh<PI))
    {
        ...
        {
                int count=1;
                while(errCurAndLastImuX>=floorH)
                {
        ...
        {
            int count=1;
            while(errCurAndLastImuY>=floorH)
            {
...
bool Location::updateAll(Mat image)
{
    ...
    hl->findLinesAndCross(image,mainLine,secondLine,cross,imuO.vth*RAD2DEG*period,ifFindMainLine,ifFindSecondLine,ifFindCross,p.fourDirection);
