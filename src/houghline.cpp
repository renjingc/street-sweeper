#include "houghline.h"
    ...
    bool HoughLine::findLinesAndCross(Mat image, Vec4f& mainLine,Vec4f& secondLine, Point &cross,double vth,
                                      bool& ifFindMainLine,bool& ifFindSecondLine,bool& ifFindCross)
    {
        ...
        getLBP();
        ifFindMainLine=findMainLine(image,image_gaussian,mainLine,vth,ifRotate);
        //寻找横线
        if(ifFindMainLine)
        {
    ...
    bool HoughLine::findMainLine(Mat frame,Mat image_gaussian,Vec4f &mainLine,double vth,int& ifRotate)
    {
        //搜第一跟线,即竖直的线
