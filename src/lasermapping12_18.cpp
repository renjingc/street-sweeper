...
void LaserMapping::startLiveSlam()
{
    ...
    //发布栅格地图
    sst2_ = node_.advertise<nav_msgs::OccupancyGrid>("pathmap", 1, true);
    //发布栅格地图信息，包括地图载入时间,分辨率(m/cell),宽度,高度,原点,
    sstm2_ = node_.advertise<nav_msgs::MapMetaData>("pathmap_metadata", 1, true);
...
void LaserMapping::publishCmd()
{
    ...
    pPathControl->updatePlan(*g_psmap,
                             map1_.map,
                             currPose,
                             poseVector[poseVector.size()-1],
                             &(laserVector[poseVector.size()-1][0]),
                             vel_cmd);
    cmd_.publish(vel_cmd);
    //cout<<vel_cmd.linear.x<<" "<<vel_cmd.angular.z<<endl;
    sst2_.publish(pPathControl->showMap);
    sstm2_.publish(pPathControl->showMap.info);
...
void LaserMapping::updateMap(const sensor_msgs::LaserScan &scan)
{
    ...
    if(!got_map_) {
      ...
      map1_.map.info.resolution = delta1_;
      map1_.map.info.origin.position.x = 0.0;
      map1_.map.info.origin.position.y = 0.0;
      map1_.map.info.origin.position.z = 0.0;
      map1_.map.info.origin.orientation.x = 0.0;
      map1_.map.info.origin.orientation.y = 0.0;
      map1_.map.info.origin.orientation.z = 0.0;
      map1_.map.info.origin.orientation.w = 1.0;
    }

    //map的中心
    LMapping::Point center,center1;
    center.x=(xmin_ + xmax_) / 2.0;
    center.y=(ymin_ + ymax_) / 2.0;

    center1.x=(xmin1_ + xmax1_) / 2.0;
    center1.y=(ymin1_ + ymax1_) / 2.0;

    //建立ScanMap类
    LMapping::ScanMap smap(center, xmin_, ymin_, xmax_, ymax_,
                                  delta_);
    LMapping::ScanMap smap1(center1, xmin1_, ymin1_, xmax1_, ymax1_,
                                  delta1_);
    for(int i=poseVector.size()-1;i>=0;i--)
    {
        ...
        computeActiveArea(smap1, poseVector[i], &(laserVector[i][0]));
        registerScan(smap1, poseVector[i], &(laserVector[i][0]));
    }
    ...
    if(map1_.map.info.width != (unsigned int) smap1.getMapSizeX() || map1_.map.info.height != (unsigned int) smap1.getMapSizeY())
    {
      LMapping::Point wmin = smap1.map2world(LMapping::IntPoint(0, 0));
      LMapping::Point wmax = smap1.map2world(LMapping::IntPoint(smap1.getMapSizeX(), smap1.getMapSizeY()));
      xmin1_ = wmin.x; ymin1_ = wmin.y;
      xmax1_ = wmax.x; ymax1_ = wmax.y;

      ROS_DEBUG("map size is now %dx%d pixels (%f,%f)-(%f, %f)", smap1.getMapSizeX(), smap1.getMapSizeY(),
                xmin1_, ymin1_, xmax1_, ymax1_);

      map1_.map.info.width = smap1.getMapSizeX();
      map1_.map.info.height = smap1.getMapSizeY();
      map1_.map.info.origin.position.x = xmin1_;
      map1_.map.info.origin.position.y = ymin1_;
      map1_.map.data.resize(map1_.map.info.width*map1_.map.info.height);

      ROS_DEBUG("map origin: (%f, %f)", map1_.map.info.origin.position.x, map1_.map.info.origin.position.y);
    }
    ...
    for(int x=0; x < smap.getMapSizeX(); x++)
    {
      for(int y=0; y < smap.getMapSizeY(); y++)
      {
        ...
        else if(occ > occ_thresh_)
        {
          ...
          //100为障碍
          map_.map.data[MAP_IDX(map_.map.info.width, x, y)] = 250;
        }
        else
          //0为可同行区域
    ...
    for(int x=0; x < smap1.getMapSizeX(); x++)
    {
      for(int y=0; y < smap1.getMapSizeY(); y++)
      {
        /// @todo Sort out the unknown vs. free vs. obstacle thresholding
        LMapping::IntPoint p(x, y);
        double occ=smap1.cell(p);
        assert(occ <= 1.0);
        //occ<0代表未知区域，
        if(occ < 0)
          map1_.map.data[MAP_IDX(map1_.map.info.width, x, y)] = -1;
        else if(occ > occ_thresh_)
        {
          //map_.map.data[MAP_IDX(map_.map.info.width, x, y)] = (int)round(occ*100.0);
          //100为障碍
          map1_.map.data[MAP_IDX(map1_.map.info.width, x, y)] = 80;
        }
        else
          //0为可同行区域
          map1_.map.data[MAP_IDX(map1_.map.info.width, x, y)] = 0;
      }
    }

    got_map_ = true;
    g_psmap=new LMapping::ScanMap(smap1.getCenter(),xmin1_, ymin1_, xmax1_, ymax1_,
                                  delta1_);
    ...
    map1_.map.header.stamp = ros::Time::now();
    map1_.map.header.frame_id = tf_.resolve( map_frame_ );

    sst_.publish(map_.map);
    sstm_.publish(map_.map.info);
    sst1_.publish(map1_.map);
    sstm1_.publish(map1_.map.info);
